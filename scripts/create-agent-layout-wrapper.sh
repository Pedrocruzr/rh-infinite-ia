...
read -rp "Slug exato do agente (igual à URL e à API): " AGENT_SLUG
read -rp "Nome do componente da página (ex: MentorDinamicasPage): " COMPONENT_NAME
read -rp "Nome completo do stacker (ex: Diagnóstico, Recrutamento & Seleção, Desenvolvimento & Cultura): " AGENT_STACKER_NAME
read -rp "Título do agente: " AGENT_TITLE
read -rp "Subtítulo do agente: " AGENT_SUBTITLE

TARGET_DIR="src/app/app/agentes/${AGENT_SLUG}"
TARGET_FILE="${TARGET_DIR}/page.tsx"

mkdir -p "${TARGET_DIR}"

cat > "${TARGET_FILE}" <<PAGE
"use client";
...
export default function ${COMPONENT_NAME}() {
  ...
  async function startConversation() {
    try {
      ...
      const response = await fetch("/api/agents/${AGENT_SLUG}", {
        method: "POST",
        headers: {
  ...
  return (
    <StandardAgentLayout
      stackerName="${AGENT_STACKER_NAME}"
      title="${AGENT_TITLE}"
...
echo "Template aplicado em: ${TARGET_FILE}"
