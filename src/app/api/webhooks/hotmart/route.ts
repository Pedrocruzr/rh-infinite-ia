...
export async function POST(request: Request) {
  try {
    // 1. Auth check using x-hotmart-hottok header
    const token = request.headers.get("x-hotmart-hottok") || "";
    const configuredSecret = process.env.HOTMART_WEBHOOK_SECRET;

    if (configuredSecret && token !== configuredSecret) {
      return NextResponse.json(
        { error: "Webhook não autorizado. Token hottok incorreto." },
        { status: 401 }
      );
    }
    ...
    // Hotmart purchase approved events
    const isApproved = eventType === "PURCHASE_APPROVED" || eventType === "PURCHASE_COMPLETE";

    if (!email || !transactionId) {
      return NextResponse.json({
        ok: false,
        warning: "Campos obrigatórios (email ou transaction) ausentes no payload.",
      });
    }
    ...
    if (!isApproved) {
      return NextResponse.json({
        ok: true,
        message: `Evento ignorado: tipo de evento é ${eventType}.`,
      });
    }

    // 3. Connect to Supabase as Admin to process purchase
    const supabase = createAdminClient();
    const { data: rpcData, error: rpcError } = await supabase.rpc("process_hotmart_payment_event", {
    ...
    if (hasEmailConfig && (rpcResult?.status === "saved_for_later_registration" || rpcResult?.status === "processed_immediately")) {
      try {
          ...
          emailHtml = `
            <div style="font-family: sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff;">
              ...
              <div style="margin: 30px 0; text-align: center;">
                <a href="https://app.stackercompany.com.br/cadastro?email=${encodeURIComponent(email)}&name=${encodeURIComponent(welcomeName)}&plan=${isProfileTest ? "perfil" : "completo"}" style="background-color: #0284c7; color: #ffffff; padding: 14px 28px; text-decoration: none; font-weight: bold; border-radius: 8px; display: inline-block;">Concluir meu Cadastro</a>
              </div>
        ...
        console.error("Erro ao enviar e-mail de boas-vindas (Hotmart Webhook):", emailError);
      }
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Erro inesperado no processamento do webhook Hotmart.",
      },
      { status: 500 }
    );
  }
